...
func TestClassifyError_AlreadyClassified(t *testing.T) {
	err := &ClassifiedError{Code: "pg_error_57", Msg: "canceling statement due to user request"}
	code, msg := ClassifyError(err)
	assert.Equal(t, "pg_error_57", code)
	assert.Equal(t, "canceling statement due to user request", msg)
}

func TestClassifyError_WrappedClassifiedError(t *testing.T) {
	inner := &ClassifiedError{Code: "timeout", Msg: "context deadline exceeded"}
	wrapped := fmt.Errorf("explicit check failed: %w", inner)
	code, msg := ClassifyError(wrapped)
	assert.Equal(t, "timeout", code)
	assert.Equal(t, "context deadline exceeded", msg)
}
...
func TestSanitizeErrorMessage(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "PostgreSQL connection URL with password",
			input:    `failed to connect to "postgres://myuser:s3cretP@ss@localhost:5432/mydb": connection refused`,
			expected: `failed to connect to "postgres://myuser:***@localhost:5432/mydb": connection refused`,
		},
		{
			name:     "Simple postgres URL",
			input:    "postgres://admin:hunter2@db.example.com:5432/production",
		...
		},
		{
			...
			input:    "postgresql://root:abc123@pghost:5432/testdb: timeout",
			expected: "postgresql://root:***@pghost:5432/testdb: timeout",
		},
	}
...
func TestClassifyError_SanitizesCredentials(t *testing.T) {
	err := errors.New(`failed to connect to "postgres://myuser:DBpwd1234@localhost:5432/mydb": connection refused`)
	code, msg := ClassifyError(err)
	assert.Equal(t, "connection_refused", code)
	assert.NotContains(t, msg, "DBpwd1234")
	assert.Contains(t, msg, "myuser:***@")
}

...
type ArgumentList struct {
	...
	CollectConnectionTiming              bool   `default:"false" help:"If true, measures DNS lookup and TCP connect time via the connection's DialFunc and reports them in PostgresqlHealthSample. No extra round-trip is issued; timing is captured on the first query of each cycle."`
	EnableAvailabilityCheck              bool   `default:"false" help:"If true, runs an explicit canary query each cycle to verify the server can process queries, reported in PostgresqlHealthSample."`
